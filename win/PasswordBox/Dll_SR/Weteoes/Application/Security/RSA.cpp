...
std::string RSAClass::B_PrivateUnEncode(std::string data) {
	char* result_c;
	int result_len = WeteoesDll::Base64_UnEncode((char*)data.c_str(), (int)data.length(), result_c);
	string result = RSA_PrivateUnEncode(string(result_c, result_len), A_PrivateKey);
	return result;
}

std::string RSAClass::Encode(std::string data, std::string key) {
	std::string result = RSA_PublicEncode(data, key);
	std::string result_64 = WeteoesDll::Base64_Encode((char*)result.c_str(), (int)result.length());
	return result_64;
}
...
std::string RSAClass::UnEncode(std::string data, std::string key) {
	char* result_c;
	int result_len = WeteoesDll::Base64_UnEncode((char*)data.c_str(), (int)data.length(), result_c);
	std::string result = RSA_PrivateUnEncode(string(result_c, result_len), key);
	return result;
}

void RSAClass::GetChars(std::string data_s, char *&data_c) { //std::string to char*
	data_c = (char*)malloc(sizeof(char) * data_s.length());
