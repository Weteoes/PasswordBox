...
#include <Weteoes/More/CEF/Application/Dlg/ChangePassword/CEF_ChangePassword_App.h>
...
void ChangePassword_Dlg::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
}

// 托盘重启
BEGIN_MESSAGE_MAP(ChangePassword_Dlg, CDialogEx)
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_WM_NCHITTEST()
ON_WM_ACTIVATE()
...
// ChangePassword_Dlg 消息处理程序

BOOL ChangePassword_Dlg::OnInitDialog() {
...
void ChangePassword_Dlg::OnPaint() {
	if (IsIconic())
	{
...
// 初始化
void ChangePassword_Dlg::Ready() {
	Ready_Dlg();
	Ready_Variable();
	VariableClass::app_Dll_SWR.Start(0);
	Ready_CEF();
}
void ChangePassword_Dlg::Ready_Variable() {
	VariableClass::dlg_CEF = dlg_CEF;
	VariableClass::dlg_HWND = dlg_HWND;
}

// 初始化窗口
void ChangePassword_Dlg::Ready_Dlg() {
	// 初始化变量
	dlg_CEF = IDC_ChangePassword_CEF;
	dlg_HWND = this->m_hWnd;

	// 设置窗口标题(用于Dll获取主窗口句柄) Title
	SetWindowText("ChangePassword");
...
// 初始化CEF
void ChangePassword_Dlg::Ready_CEF() {
	...
	GetDlgItem(dlg_CEF)->GetClientRect(&CEF_ChangePassword_App::CEF_CRect);
	CEF_ChangePassword_App::CEF_HWND = GetSafeHwnd();
...
// Alt + F4
void ChangePassword_Dlg::OnCancel() {
	VariableClass::appDlgClass.Minimize();
	//CDialogEx::OnCancel();
}

void ChangePassword_Dlg::OnActivate(UINT nState, CWnd* pWndOther, BOOL bMinimized) {
	Ready_Variable();
...
BOOL ChangePassword_Dlg::OnHelpInfo(HELPINFO* pHelpInfo) {
	return 0;
}
